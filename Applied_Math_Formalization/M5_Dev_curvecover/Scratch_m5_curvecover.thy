...
text \<open>The \<open>u\<close>-fibre (\<open>\<omega>\<^sub>1\<close>) derivative: restricting \<open>crossTheta\<close> to the HORIZONTAL line
  \<open>u \<mapsto> (u,s)\<close> gives, via the separable form, \<open>crossA(u)\<cdot>cos s + crossB(u)\<cdot>sin s +
  crossG(u)\<close>, now a function of the running variable \<open>u\<close> through \<open>cos u\<close>/\<open>sin u\<close>.  Its
  derivative is \<open>\<partial>\<^sub>1\<close>, the partial whose nonvanishing makes the vertical graph map
  @{term "\<lambda>z. vector [crossTheta \<omega>0 \<omega>s z, z$2]"} injective.\<close>

lemma has_field_derivative_crossTheta_s:
  fixes \<omega>0 \<omega>s :: "real^2" and s t :: real
  defines "Ac \<equiv> (kx \<omega>0 - kx \<omega>s)/(kz \<omega>s - kz \<omega>0)"
      and "Bc \<equiv> (ky \<omega>0 - ky \<omega>s)/(kz \<omega>s - kz \<omega>0)"
  shows "((\<lambda>u. crossTheta \<omega>0 \<omega>s (vector [u, s])) has_field_derivative
          ((Bc * kz \<omega>s + ky \<omega>s) * sin t * cos s
           - (Ac * kz \<omega>s + kx \<omega>s) * sin t * sin s
           + (Ac * ky \<omega>s - Bc * kx \<omega>s) * cos t)) (at t)"
proof -
  have sep: "crossTheta \<omega>0 \<omega>s (vector [u, s])
    ...
    unfolding Ac_def Bc_def
    by (subst crossTheta_separable) (simp add: vector_2 crossA_def crossB_def crossG_def)
  show ?thesis
    unfolding sep
    by (auto intro!: derivative_eq_intros simp: algebra_simps)
qed

text \<open>The vertical graph map \<open>z \<mapsto> (crossTheta z, z$2)\<close> has derivative \<open>h \<mapsto> (T h, h$2)\<close>
  whenever \<open>crossTheta\<close> has derivative \<open>T\<close> --- the symmetric component-assembly helper.\<close>

lemma has_derivative_graph_map_vert:
  fixes \<omega>0 \<omega>s z :: "real^2" and T :: "real^2 \<Rightarrow> real"
  assumes "((\<lambda>z. crossTheta \<omega>0 \<omega>s z) has_derivative T) (at z)"
  shows "((\<lambda>z. vector [crossTheta \<omega>0 \<omega>s z, z$2] :: real^2) has_derivative
            (\<lambda>h. vector [T h, h$2])) (at z)"
proof -
  have e: "(\<lambda>z. vector [crossTheta \<omega>0 \<omega>s z, z$2] :: real^2)
         = (\<lambda>z. (crossTheta \<omega>0 \<omega>s z) *\<^sub>R axis 1 1 + (z$2) *\<^sub>R axis 2 1)"
    by (rule ext)
       (simp add: Finite_Cartesian_Product.vec_eq_iff forall_2 vector_2
                  vector_add_component vector_scaleR_component axis_def)
  have d2: "((\<lambda>z::real^2. z$2) has_derivative (\<lambda>h. h$2)) (at z)"
    by (simp add: bounded_linear_vec_nth bounded_linear_imp_has_derivative)
  have hd: "((\<lambda>z. (crossTheta \<omega>0 \<omega>s z) *\<^sub>R axis 1 1 + (z$2) *\<^sub>R (axis 2 1 :: real^2))
...
text \<open>\<^bold>\<open>Injectivity of the vertical graph map.\<close>  Where \<open>\<partial>\<^sub>1 \<noteq> 0\<close>, the vertical graph map
  \<open>Hv z = (crossTheta z, z$2)\<close> is injective on a small ball: the second coordinate pins
  \<open>z$2\<close>, and on the horizontal fibre Rolle's theorem forbids two \<open>\<omega>\<^sub>1\<close>-values with equal
  \<open>crossTheta\<close>.  This is the symmetric hypothesis for invariance of domain.\<close>

lemma crossTheta_graph_inj_vert:
  fixes \<omega>0 \<omega>s \<omega>' :: "real^2"
  defines "Ac \<equiv> (kx \<omega>0 - kx \<omega>s)/(kz \<omega>s - kz \<omega>0)"
      and "Bc \<equiv> (ky \<omega>0 - ky \<omega>s)/(kz \<omega>s - kz \<omega>0)"
  ...
  shows "\<exists>\<epsilon>>0. inj_on (\<lambda>z. vector [crossTheta \<omega>0 \<omega>s z, z$2] :: real^2) (ball \<omega>' \<epsilon>)
          \<and> (\<forall>z\<in>ball \<omega>' \<epsilon>.
...
proof -
  ...
  have ftd: "((\<lambda>u. crossTheta \<omega>0 \<omega>s (vector [u, s])) has_field_derivative D1 (vector [u, s])) (at u)"
    for s u :: real
  proof -
    have "((\<lambda>u'. crossTheta \<omega>0 \<omega>s (vector [u', s]))
      ...
      unfolding Ac_def Bc_def by (rule has_field_derivative_crossTheta_s)
  ...
  proof -
    ...
    thus ?thesis
      using zb z'b convex_ball convex_contains_segment by blast
  qed
  have inj: "inj_on (\<lambda>z. vector [crossTheta \<omega>0 \<omega>s z, z$2] :: real^2) (ball \<omega>' \<epsilon>)"
  proof (rule inj_onI)
    fix z z' :: "real^2" assume zb: "z \<in> ball \<omega>' \<epsilon>" and z'b: "z' \<in> ball \<omega>' \<epsilon>"
      and Heq: "vector [crossTheta \<omega>0 \<omega>s z, z$2] = (vector [crossTheta \<omega>0 \<omega>s z', z'$2] :: real^2)"
    ...
    proof (rule ccontr)
      ...
      define f where "f = (\<lambda>u. crossTheta \<omega>0 \<omega>s (vector [u, z$2]))"
      have fz: "f (z$1) = crossTheta \<omega>0 \<omega>s z" using zrw by (simp add: f_def)
      have fz': "f (z'$1) = crossTheta \<omega>0 \<omega>s z'" using z'rw s_eq by (simp add: f_def)
...
text \<open>Injectivity + invariance of domain: the vertical graph map \<open>Hv z = (crossTheta z, z$2)\<close>
  is a homeomorphism of a small ball onto an OPEN image, with continuous inverse \<open>g\<close>.\<close>

lemma crossTheta_graph_homeo_vert:
  fixes \<omega>0 \<omega>s \<omega>' :: "real^2"
  defines "Ac \<equiv> (kx \<omega>0 - kx \<omega>s)/(kz \<omega>s - kz \<omega>0)"
      and "Bc \<equiv> (ky \<omega>0 - ky \<omega>s)/(kz \<omega>s - kz \<omega>0)"
  assumes d1: "(Bc * kz \<omega>s + ky \<omega>s) * sin (\<omega>'$1) * cos (\<omega>'$2)
  ...
  obtains \<epsilon> g where "\<epsilon> > 0"
    "homeomorphism (ball \<omega>' \<epsilon>)
        ((\<lambda>z. vector [crossTheta \<omega>0 \<omega>s z, z$2] :: real^2) ` (ball \<omega>' \<epsilon>))
        (\<lambda>z. vector [crossTheta \<omega>0 \<omega>s z, z$2]) g"
    "open ((\<lambda>z. vector [crossTheta \<omega>0 \<omega>s z, z$2] :: real^2) ` (ball \<omega>' \<epsilon>))"
    "\<forall>z\<in>ball \<omega>' \<epsilon>.
         (Bc * kz \<omega>s + ky \<omega>s) * sin (z$1) * cos (z$2)
       - (Ac * kz \<omega>s + kx \<omega>s) * sin (z$1) * sin (z$2)
       + (Ac * ky \<omega>s - Bc * kx \<omega>s) * cos (z$1) \<noteq> 0"
proof -
  obtain \<epsilon> where \<epsilon>0: "\<epsilon> > 0"
      and inj: "inj_on (\<lambda>z. vector [crossTheta \<omega>0 \<omega>s z, z$2] :: real^2) (ball \<omega>' \<epsilon>)"
      and avoid: "\<forall>z\<in>ball \<omega>' \<epsilon>.
          (Bc * kz \<omega>s + ky \<omega>s) * sin (z$1) * cos (z$2)
        - (Ac * kz \<omega>s + kx \<omega>s) * sin (z$1) * sin (z$2)
        + (Ac * ky \<omega>s - Bc * kx \<omega>s) * cos (z$1) \<noteq> 0"
    using crossTheta_graph_inj_vert[OF d1[unfolded Ac_def Bc_def]] unfolding Ac_def Bc_def by blast
  have veq: "(\<lambda>z. vector [crossTheta \<omega>0 \<omega>s z, z$2] :: real^2)
           = (\<lambda>z. (crossTheta \<omega>0 \<omega>s z) *\<^sub>R axis 1 1 + (z$2) *\<^sub>R axis 2 1)"
    by (rule ext)
       (simp add: Finite_Cartesian_Product.vec_eq_iff forall_2 vector_2
                  vector_add_component vector_scaleR_component axis_def)
  have contH: "continuous_on (ball \<omega>' \<epsilon>) (\<lambda>z. vector [crossTheta \<omega>0 \<omega>s z, z$2] :: real^2)"
    unfolding veq
    by (intro continuous_intros continuous_on_crossTheta
              linear_continuous_on[OF bounded_linear_vec_nth])
  obtain g where homeo: "homeomorphism (ball \<omega>' \<epsilon>)
        ...
        (\<lambda>z. vector [crossTheta \<omega>0 \<omega>s z, z$2]) g"
    using invariance_of_domain_homeomorphism[OF open_ball contH _ inj] by auto
  have openimg: "open ((\<lambda>z. vector [crossTheta \<omega>0 \<omega>s z, z$2] :: real^2) ` (ball \<omega>' \<epsilon>))"
    by (rule invariance_of_domain[OF contH open_ball inj])
  show ?thesis using \<epsilon>0 homeo openimg avoid by (blast intro: that)
qed
...
lemma crossTheta_local_C1_graph_vert:
  fixes \<omega>0 \<omega>s \<omega>' :: "real^2"
  ...
  obtains a b and \<psi> :: "real \<Rightarrow> real^2" and r where "a < \<omega>'$2" "\<omega>'$2 < b" "0 < r"
    ...
    "\<And>s. s \<in> {a..b} \<Longrightarrow> crossTheta \<omega>0 \<omega>s (\<psi> s) = 0"
    "\<psi> (\<omega>'$2) = \<omega>'"
    "{\<omega>. crossTheta \<omega>0 \<omega>s \<omega> = 0} \<inter> ball \<omega>' r \<subseteq> \<psi> ` {a..b}"
proof -
  define D1 :: "real^2 \<Rightarrow> real" where
    "D1 = (\<lambda>z. (Bc * kz \<omega>s + ky \<omega>s) * sin (z$1) * cos (z$2)
             - (Ac * kz \<omega>s + kx \<omega>s) * sin (z$1) * sin (z$2)
  ...
  define D2 :: "real^2 \<Rightarrow> real" where
    "D2 = (\<lambda>z. - crossA Bc \<omega>s (z$1) * sin (z$2) + crossB Ac \<omega>s (z$1) * cos (z$2))"
  define Hv :: "real^2 \<Rightarrow> real^2" where "Hv = (\<lambda>z. vector [crossTheta \<omega>0 \<omega>s z, z$2])"
  ...
  obtain \<epsilon> g where \<epsilon>0: "\<epsilon> > 0"
    ...
    by (rule crossTheta_graph_homeo_vert[OF d1[unfolded Ac_def Bc_def]])
  ...
  qed
  have psi0: "crossTheta \<omega>0 \<omega>s (\<psi> s) = 0" if "s \<in> {a..b}" for s :: real
  proof -
    have "crossTheta \<omega>0 \<omega>s (\<psi> s) = (Hv (\<psi> s)) $ 1" unfolding Hv_def by (simp add: vector_2)
  ...
  proof (rule that[where a = a and b = b and \<psi> = \<psi> and r = "min \<epsilon> (\<eta>/2)"])
    ...
    show "\<And>s. s \<in> {a..b} \<Longrightarrow> crossTheta \<omega>0 \<omega>s (\<psi> s) = 0" by (rule psi0)
    show "\<psi> (\<omega>'$2) = \<omega>'"
      unfolding \<psi>_def by (simp add: Hvw'[symmetric] homeomorphism_apply1[OF homeo w'ball])
    show "{\<omega>. crossTheta \<omega>0 \<omega>s \<omega> = 0} \<inter> ball \<omega>' (min \<epsilon> (\<eta>/2)) \<subseteq> \<psi> ` {a..b}"
    proof
      fix \<omega> :: "real^2" assume "\<omega> \<in> {\<omega>. crossTheta \<omega>0 \<omega>s \<omega> = 0} \<inter> ball \<omega>' (min \<epsilon> (\<eta>/2))"
      hence cz: "crossTheta \<omega>0 \<omega>s \<omega> = 0"
        and wbe: "dist \<omega> \<omega>' < \<epsilon>" and wbh: "dist \<omega> \<omega>' < \<eta>/2"
        by (auto simp: dist_commute min_less_iff_conj)
      have wball: "\<omega> \<in> ball \<omega>' \<epsilon>" using wbe by (simp add: dist_commute)
...
lemma crossTheta_singular_cover:
  ...
  obtains \<gamma>1 \<gamma>2 :: "real \<Rightarrow> real^2" and a1 b1 a2 b2 r :: real where
      ...
      "\<gamma>1 ` {a1..b1} \<subseteq> {x. crossTheta \<omega>0 \<omega>s x = 0}" "\<gamma>2 ` {a2..b2} \<subseteq> {x. crossTheta \<omega>0 \<omega>s x = 0}"
proof -
...
lemma crossTheta_horiz_edge_finite_or_flat:
  fixes \<omega>0 \<omega>s :: "real^2" and c lo hi :: real
  defines "Ac \<equiv> (kx \<omega>0 - kx \<omega>s)/(kz \<omega>s - kz \<omega>0)"
      and "Bc \<equiv> (ky \<omega>0 - ky \<omega>s)/(kz \<omega>s - kz \<omega>0)"
  shows "finite {u::real. lo \<le> u \<and> u \<le> hi \<and> crossTheta \<omega>0 \<omega>s (vector [u, c]) = 0}
...
proof (cases "(- (Bc * kz \<omega>s + ky \<omega>s) * cos c + (Ac * kz \<omega>s + kx \<omega>s) * sin c) \<noteq> 0
            \<or> (Ac * ky \<omega>s - Bc * kx \<omega>s) \<noteq> 0")
  case True
  have "finite {u::real. lo \<le> u \<and> u \<le> hi \<and> crossTheta \<omega>0 \<omega>s (vector [u, c]) = 0}"
...
next
  ...
  have sep: "crossTheta \<omega>0 \<omega>s (vector [u, c])
    ...
    unfolding Ac_def Bc_def
    by (subst crossTheta_separable) (simp add: vector_2 crossA_def crossB_def crossG_def algebra_simps)
  have const: "crossTheta \<omega>0 \<omega>s (vector [u, c]) = (Bc * cos c - Ac * sin c)" for u :: real
    using sep a0 b0 by simp
  show ?thesis
  proof (cases "(Bc * cos c - Ac * sin c) = 0")
    case True
    hence "\<forall>u::real. crossTheta \<omega>0 \<omega>s (vector [u, c]) = 0" using const by simp
    thus ?thesis by blast
  next
    case False
    have "crossTheta \<omega>0 \<omega>s (vector [u, c]) \<noteq> 0" for u :: real using const False by simp
    hence "{u::real. lo \<le> u \<and> u \<le> hi \<and> crossTheta \<omega>0 \<omega>s (vector [u, c]) = 0} \<subseteq> {}" by auto
    from finite_subset[OF this finite.emptyI] show ?thesis by blast
  qed
qed

lemma crossTheta_vert_edge_finite_or_flat:
  fixes \<omega>0 \<omega>s :: "real^2" and c lo hi :: real
  defines "Ac \<equiv> (kx \<omega>0 - kx \<omega>s)/(kz \<omega>s - kz \<omega>0)"
      and "Bc \<equiv> (ky \<omega>0 - ky \<omega>s)/(kz \<omega>s - kz \<omega>0)"
  shows "finite {u::real. lo \<le> u \<and> u \<le> hi \<and> crossTheta \<omega>0 \<omega>s (vector [c, u]) = 0}
       \<or> (\<forall>u::real. crossTheta \<omega>0 \<omega>s (vector [c, u]) = 0)"
proof (cases "crossA Bc \<omega>s c \<noteq> 0 \<or> crossB Ac \<omega>s c \<noteq> 0")
  case True
  have "finite {u::real. lo \<le> u \<and> u \<le> hi \<and> crossTheta \<omega>0 \<omega>s (vector [c, u]) = 0}"
    by (rule finite_locus_vert_edge[OF True[unfolded Ac_def Bc_def]])
  thus ?thesis by blast
next
  case False
  hence a0: "crossA Bc \<omega>s c = 0" and b0: "crossB Ac \<omega>s c = 0" by auto
  have sep: "crossTheta \<omega>0 \<omega>s (vector [c, u])
           = crossA Bc \<omega>s c * cos u + crossB Ac \<omega>s c * sin u + crossG Ac Bc \<omega>s c" for u :: real
    unfolding Ac_def Bc_def by (subst crossTheta_separable) (simp add: vector_2)
  have const: "crossTheta \<omega>0 \<omega>s (vector [c, u]) = crossG Ac Bc \<omega>s c" for u :: real
    using sep a0 b0 by simp
  show ?thesis
  proof (cases "crossG Ac Bc \<omega>s c = 0")
    case True
    hence "\<forall>u::real. crossTheta \<omega>0 \<omega>s (vector [c, u]) = 0" using const by simp
    thus ?thesis by blast
  next
    case False
    have "crossTheta \<omega>0 \<omega>s (vector [c, u]) \<noteq> 0" for u :: real using const False by simp
    hence "{u::real. lo \<le> u \<and> u \<le> hi \<and> crossTheta \<omega>0 \<omega>s (vector [c, u]) = 0} \<subseteq> {}" by auto
    from finite_subset[OF this finite.emptyI] show ?thesis by blast
  qed
qed
...
lemma crossTheta_boundary_cover_phi:
...
proof -
  ...
  have fv2l: "finite {s \<in> {a0..b0}. \<phi> s $ 2 = u$2}"
    unfolding u2
    by (rule crossTheta_arc_horiz_face_finite[OF inj phicz bdd1 ndbot[unfolded Ac_def Bc_def]], auto)
  have fv2h: "finite {s \<in> {a0..b0}. \<phi> s $ 2 = v$2}"
    unfolding v2
    by (rule crossTheta_arc_horiz_face_finite[OF inj phicz bdd1 ndtop[unfolded Ac_def Bc_def]])
...
lemma crossTheta_boundary_cover_chi:
  ...
  assumes win: "\<omega>' \<in> OmegaPF ctr \<delta>"
    ...
    and cov0: "{\<omega>. crossTheta \<omega>0 \<omega>s \<omega> = 0} \<inter> ball \<omega>' r0 \<subseteq> \<psi> ` {a0..b0}"
    and ndleft: "crossA Bc \<omega>s (ctr$1 - \<delta>) \<noteq> 0 \<or> crossB Ac \<omega>s (ctr$1 - \<delta>) \<noteq> 0"
    and ndright: "crossA Bc \<omega>s (ctr$1 + \<delta>) \<noteq> 0 \<or> crossB Ac \<omega>s (ctr$1 + \<delta>) \<noteq> 0"
  obtains r \<A> where "0 < r" "finite \<A>"
      "\<forall>\<gamma>\<in>\<A>. analytic_arc \<gamma> \<and> \<gamma> \<subseteq> OmegaPF ctr \<delta>"
      "{\<omega> \<in> OmegaPF ctr \<delta>. crossTheta \<omega>0 \<omega>s \<omega> = 0} \<inter> ball \<omega>' r \<subseteq> \<Union>\<A>"
proof -
  ...
  have fv1l: "finite {s \<in> {a0..b0}. \<psi> s $ 1 = u$1}"
    unfolding u1
    by (rule crossTheta_arc_vert_face_finite[OF inj psicz bdd2 ndleft[unfolded Ac_def Bc_def]], auto)
  have fv1h: "finite {s \<in> {a0..b0}. \<psi> s $ 1 = v$1}"
    unfolding v1
    by (rule crossTheta_arc_vert_face_finite[OF inj psicz bdd2 ndright[unfolded Ac_def Bc_def]], auto)
  obtain P :: "(real\<times>real) set" where finP: "finite P"
          ...
          next
            ...
            next
              ...
              \<comment> \<open>\<^bold>\<open>\<open>R = 0 \<and> \<partial>\<^sub>2\<Theta> = 0 \<and> \<partial>\<^sub>1\<Theta> = 0\<close> (singular at the collinear-wavevector
                  ...
                  (@{thm crossTheta_boundary_singular_cover}), EXCEPT each face's flat branch is
              ...
              \<comment> \<open>STEP 1: flat + indef.  Copied verbatim from @{thm crossTheta_boundary_singular_cover};
              ...
              have d2zAc: "- crossA Bc \<omega>s (\<omega>'$1) * sin (\<omega>'$2) + crossB Ac \<omega>s (\<omega>'$1) * cos (\<omega>'$2) = 0"
              ...
              have g2: "(Gabs Ac Bc (kx \<omega>s) (ky \<omega>s) (kz \<omega>s) \<omega>')$2 = 0"
                using d2zAc by (simp add: Gabs_def vector_2 crossA_def crossB_def algebra_simps)
              have flat: "Gabs Ac Bc (kx \<omega>s) (ky \<omega>s) (kz \<omega>s) \<omega>' = 0"
                using g1 g2 by (simp add: Finite_Cartesian_Product.vec_eq_iff forall_2 zero_index)
              have pyth2: "cos (\<omega>'$2) * cos (\<omega>'$2) + sin (\<omega>'$2) * sin (\<omega>'$2) = 1"
                by (simp add: field_simps sin_cos_squared_add)
              ...
              have th0: "(Bc - P * cos (\<omega>'$1)) * cos (\<omega>'$2) + (- Ac + Q * cos (\<omega>'$1)) * sin (\<omega>'$2)
                ...
                by (subst (asm) crossTheta_separable)
                   (simp add: crossA_def crossB_def crossG_def Ac_def Bc_def algebra_simps)
              have d2': "- (Bc - P * cos (\<omega>'$1)) * sin (\<omega>'$2) + (- Ac + Q * cos (\<omega>'$1)) * cos (\<omega>'$2) = 0"
                using d2zAc unfolding P_def Q_def by (simp add: crossA_def crossB_def algebra_simps)
              have d1': "(P * sin (\<omega>'$1)) * cos (\<omega>'$2) + (- Q * sin (\<omega>'$1)) * sin (\<omega>'$2) + R * cos (\<omega>'$1) = 0"
              ...
              \<comment> \<open>Dispatch on whether all four box-edge LINES are non-flat.  (The line
                  dichotomy @{thm crossTheta_vert_edge_finite_or_flat} /
                  @{thm crossTheta_horiz_edge_finite_or_flat} is used implicitly: in the
                  MAIN case each face's \<open>flat\<close> alternative is the negated hypothesis here.)\<close>
              show ?thesis
              proof (cases "\<not> (\<forall>uu::real. crossTheta \<omega>0 \<omega>s (vector [u$1, uu]) = 0)
                          \<and> \<not> (\<forall>uu::real. crossTheta \<omega>0 \<omega>s (vector [v$1, uu]) = 0)
                          \<and> \<not> (\<forall>uu::real. crossTheta \<omega>0 \<omega>s (vector [uu, u$2]) = 0)
                          \<and> \<not> (\<forall>uu::real. crossTheta \<omega>0 \<omega>s (vector [uu, v$2]) = 0)")
                case True
                ...
                from True have nVl: "\<not> (\<forall>uu::real. crossTheta \<omega>0 \<omega>s (vector [u$1, uu]) = 0)"
                  and nVr: "\<not> (\<forall>uu::real. crossTheta \<omega>0 \<omega>s (vector [v$1, uu]) = 0)"
                  and nHb: "\<not> (\<forall>uu::real. crossTheta \<omega>0 \<omega>s (vector [uu, u$2]) = 0)"
                  and nHt: "\<not> (\<forall>uu::real. crossTheta \<omega>0 \<omega>s (vector [uu, v$2]) = 0)" by blast+
                show ?thesis
                proof (rule crossTheta_singular_cover[OF onepos z0 flat[unfolded Ac_def Bc_def] indef[unfolded Ac_def Bc_def]])
                  fix \<gamma>1 \<gamma>2 :: "real \<Rightarrow> real^2" and a1 b1 a2 b2 r :: real
                  assume r0: "0 < r" and ab1: "a1 \<le> b1" and ab2: "a2 \<le> b2"
                  ...
                  have fv1l_1: "finite {s \<in> {a1..b1}. \<gamma>1 s $ 1 = u$1}"
                    using crossTheta_arc_vert_face_finite_or_flat[OF inj1 lpt1 bdd1_2, of "u$1"] nVl by blast
                  have fv1h_1: "finite {s \<in> {a1..b1}. \<gamma>1 s $ 1 = v$1}"
                    using crossTheta_arc_vert_face_finite_or_flat[OF inj1 lpt1 bdd1_2, of "v$1"] nVr by blast
                  have fh1l_1: "finite {s \<in> {a1..b1}. \<gamma>1 s $ 2 = u$2}"
                    using crossTheta_arc_horiz_face_finite_or_flat[OF inj1 lpt1 bdd1_1, of "u$2"] nHb by blast
                  have fh1h_1: "finite {s \<in> {a1..b1}. \<gamma>1 s $ 2 = v$2}"
                    using crossTheta_arc_horiz_face_finite_or_flat[OF inj1 lpt1 bdd1_1, of "v$2"] nHt by blast
                  \<comment> \<open>the four faces of \<open>\<gamma>2\<close> are finite\<close>
                  have fv2l_2: "finite {s \<in> {a2..b2}. \<gamma>2 s $ 1 = u$1}"
                    using crossTheta_arc_vert_face_finite_or_flat[OF inj2 lpt2 bdd2_2, of "u$1"] nVl by blast
                  have fv2h_2: "finite {s \<in> {a2..b2}. \<gamma>2 s $ 1 = v$1}"
                    using crossTheta_arc_vert_face_finite_or_flat[OF inj2 lpt2 bdd2_2, of "v$1"] nVr by blast
                  have fh2l_2: "finite {s \<in> {a2..b2}. \<gamma>2 s $ 2 = u$2}"
                    using crossTheta_arc_horiz_face_finite_or_flat[OF inj2 lpt2 bdd2_1, of "u$2"] nHb by blast
                  have fh2h_2: "finite {s \<in> {a2..b2}. \<gamma>2 s $ 2 = v$2}"
                    using crossTheta_arc_horiz_face_finite_or_flat[OF inj2 lpt2 bdd2_1, of "v$2"] nHt by blast
                  \<comment> \<open>clip each branch to the box\<close>
                  obtain P1 :: "(real\<times>real) set" where finP1: "finite P1"
                      and P1eq: "{s \<in> {a1..b1}. \<gamma>1 s \<in> cbox u v} = (\<Union>(lo,hi)\<in>P1. {lo..hi})"
                      and P1bd: "\<forall>(lo,hi)\<in>P1. a1 \<le> lo \<and> lo \<le> hi \<and> hi \<le> b1 \<and> \<gamma>1 C1_differentiable_on {lo..hi}"
