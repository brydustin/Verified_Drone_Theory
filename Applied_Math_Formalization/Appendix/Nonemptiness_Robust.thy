...
proof -
  have key: "\<And>n.
       ...
       =
       phase c0_paper y n
         * ((of_real ((y $ n) $ 1))\<^sup>2 * (of_real (T $ 1 $ 1))\<^sup>2)
       + 2 *
         (phase c0_paper y n
           * (of_real (w_M12 (y $ n))
             * (of_real (T $ 1 $ 1) * of_real (T $ 1 $ 2))))
       + phase c0_paper y n
         * ((of_real ((y $ n) $ 2))\<^sup>2 * (of_real (T $ 1 $ 2))\<^sup>2)"
  proof -
    fix n
    have ph: "phase c (applyT T y) n = phase c0_paper y n"
      by (rule phase_applyT[OF assms])
    have lin: "(applyT T y $ n) $ 1 =
          T $ 1 $ 1 * (y $ n) $ 1 + T $ 1 $ 2 * (y $ n) $ 2"
      by (simp add: applyT_def matrix_vector_mult_def sum_2)
    show "phase c (applyT T y) n * (of_real ((applyT T y $ n) $ 1))\<^sup>2
       ...
       + phase c0_paper y n
         * ((of_real ((y $ n) $ 2))\<^sup>2 * (of_real (T $ 1 $ 2))\<^sup>2)"
      using ph lin
      by (simp add: w_M12_def power2_eq_square algebra_simps)
  qed
  ...
  show ?thesis
    ...
    using  sum_key
    by (simp add: sum_distrib_left algebra_simps power2_eq_square)
qed
...
proof -
  have key: "\<And>n.
       ...
       =
       phase c0_paper y n
         * ((of_real ((y $ n) $ 1))\<^sup>2 * (of_real (T $ 2 $ 1))\<^sup>2)
       + 2 *
         (phase c0_paper y n
           * (of_real (w_M12 (y $ n))
             * (of_real (T $ 2 $ 1) * of_real (T $ 2 $ 2))))
       + phase c0_paper y n
         * ((of_real ((y $ n) $ 2))\<^sup>2 * (of_real (T $ 2 $ 2))\<^sup>2)"
  proof -
    fix n
    have ph: "phase c (applyT T y) n = phase c0_paper y n"
      by (rule phase_applyT[OF assms])
    have lin: "(applyT T y $ n) $ 2 =
          T $ 2 $ 1 * (y $ n) $ 1 + T $ 2 $ 2 * (y $ n) $ 2"
      by (simp add: applyT_def matrix_vector_mult_def sum_2)
...
proof -
  have key: "\<And>n. of_real (w_M12 (applyT T y $ n)) * phase c (applyT T y) n
       = of_real ((T $ 1 $ 1) * (T $ 2 $ 1)) * (of_real (((y $ n) $ 1)\<^sup>2) * phase c0_paper y n)
       + of_real ((T $ 1 $ 1) * (T $ 2 $ 2) + (T $ 1 $ 2) * (T $ 2 $ 1)) * (of_real (w_M12 (y $ n)) * phase c0_paper
y n)
       + of_real ((T $ 1 $ 2) * (T $ 2 $ 2)) * (of_real (((y $ n) $ 2)\<^sup>2) * phase c0_paper y n)"
  proof -
    fix n
    have ph: "phase c (applyT T y) n = phase c0_paper y n"
      by (rule phase_applyT[OF assms])
    have lin1: "(applyT T y $ n) $ 1 = T $ 1 $ 1 * (y $ n) $ 1 + T $ 1 $ 2 * (y $ n) $ 2"
      by (simp add: applyT_def matrix_vector_mult_def sum_2)
    have lin2: "(applyT T y $ n) $ 2 = T $ 2 $ 1 * (y $ n) $ 1 + T $ 2 $ 2 * (y $ n) $ 2"
      by (simp add: applyT_def matrix_vector_mult_def sum_2)
    have wt: "w_M12 (applyT T y $ n)
            = (T $ 1 $ 1) * (T $ 2 $ 1) * ((y $ n) $ 1)\<^sup>2
            + ((T $ 1 $ 1) * (T $ 2 $ 2) + (T $ 1 $ 2) * (T $ 2 $ 1)) * w_M12 (y $ n)
            + (T $ 1 $ 2) * (T $ 2 $ 2) * ((y $ n) $ 2)\<^sup>2"
      unfolding w_M12_def lin1 lin2
      by (simp add: power2_eq_square algebra_simps)
    show "of_real (w_M12 (applyT T y $ n)) * phase c (applyT T y) n
       = of_real ((T $ 1 $ 1) * (T $ 2 $ 1)) * (of_real (((y $ n) $ 1)\<^sup>2) * phase c0_paper y n)
       + of_real ((T $ 1 $ 1) * (T $ 2 $ 2) + (T $ 1 $ 2) * (T $ 2 $ 1)) * (of_real (w_M12 (y $ n)) * phase c0_paper
y n)
       + of_real ((T $ 1 $ 2) * (T $ 2 $ 2)) * (of_real (((y $ n) $ 2)\<^sup>2) * phase c0_paper y n)"
      by (simp add: ph wt of_real_add of_real_mult algebra_simps)
  qed
  show ?thesis
    unfolding M11_moment_def M12_moment_def M22_moment_def
    by (simp add: key sum.distrib sum_distrib_left mult.assoc)
