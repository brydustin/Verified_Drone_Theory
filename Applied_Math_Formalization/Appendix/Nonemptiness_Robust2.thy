...
section \<open>Robust feasible set, part 2 (steering transport \<open>\<longrightarrow>\<close> capstone)\<close>
...
text \<open>\<^bold>\<open>(M4) Regular stratum is meager.\<close>  On the open locus where \<open>A \<noteq> 0\<close>, \<open>surj (DM_paper_x \<dots>)\<close>,
  ...
  the parametric transversality engine (instantiated directly at \<open>G = \<nabla>\<^sub>\<Omega>U\<close>) on each, the degenerate-critical projection
