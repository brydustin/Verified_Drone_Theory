...
text \<open>\<^bold>\<open>(M5) stub D34 --- the residual rank-deficient stratum.\<close>
  ...
  \<Rightarrow> subset meager) is proven here, sorry-free at the assembly layer.  The two
  internal pieces @{text m5_D34_D3_collinear} and @{text m5_D34_D4_branchP} are
  the genuine multi-week obligations and carry one precisely-scoped \<open>sorry\<close> each
  ...
  consumes; the reduction is proven here with no sorry, so the remaining
...
text \<open>These two are PROVEN sorry-free in \<open>Nonemptiness_Robust3\<close> (in scope at the
...
text \<open>The full D34 bad set injects into the residual carrying only the geometric
  ...
  existential are dropped (they only enlarge the set).  Proven sorry-free.\<close>
