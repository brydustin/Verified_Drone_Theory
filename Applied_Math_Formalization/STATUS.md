...
Status legend: ✅ proven · ◐ proven modulo a `proof hole` stub · ⬚ placeholder
...
| `prop_regzero`       | `prop:regzero`       | ◐ | meagerness reduced to `parametric_transversality_meager_stub` (proof hole) |
| `lem_foldfields`     | `lem:foldfields`     | ✅ | full (φ/θ derivatives + `det Jcvec`) |
| `prop_foldzero`      | `prop:foldzero`      | ◐ | finite-union meagerness proven; per-chart `chart_zero_projection_meager_stub` (proof hole) |
| `lem_Efinite`        | `lem:Efinite`        | ◐ | **stated truly** (`E` finite); real-analyticity modeled as the real restriction of an entire function; the isolated-zeros step is the remaining `proof hole` |
...
`Topology_Bridge.thy` (0 proof hole), and the array-factor core (`af_zero_odd_not_collinear`,
...
the Sard/critical-set plumbing remain `proof hole`.
...
| 435  | `lem:Efinite`        | `E = {ω∈Σ : g_θ(ω)=0}` is finite (real-analytic zero set, ≤2 φ-solutions each). | ◐ `lem_Efinite` (stated truly; isolated-zeros `proof hole`) |
...
1. ~~Wire `thm_final` to `nonemptiness_from_meager_branches`~~ **DONE** — `thm_final`
   ...
   `nonemptiness_from_branches` was already proof-complete.)
2. ~~Transcribe the `shows True` placeholders~~ **DONE** — `lem_Efinite`,
   ...
   `proof hole`. The four `shows True` placeholders are gone; Paper.thy is down to 3
   real proof holes (the two transversality stubs + `lem_Efinite`).
